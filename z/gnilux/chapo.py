...
class U2FKey:
    def __init__(self, mappings_file: str | None = None, rp_id: str | None = None, device_index: int | None = None, secrets_dir: str = f".d/terces-{VERSION}"):
        self.mappings_file: str = os.path.expanduser(mappings_file or '/etc/u2f_mappings')
    ...
    def load_key_handle(self, key_index: int | None = None) -> str:
        """Load key handle from mappings file by index (default from CFG)"""
        from .config import CFG
        idx = key_index if key_index is not None else CFG.get("key_index", 0)
        try:
            with open(self.mappings_file) as f:
                lines = [l.strip() for l in f if l.strip()]
            if idx >= len(lines):
                _error(f"key_index {idx} out of range ({len(lines)} keys)")
                raise IndexError(f"key_index {idx} not found")
            return lines[idx].split(':')[1].split(',')[0]
    ...
    def get_terces(self, salt: bytes, pin_token=None) -> bytes:
        ...
        if pin_token is None:
            ...
            try:
                ...
                if e.code == CtapError.ERR.PIN_INVALID:
                    retries_left = client_pin.get_pin_retries()
                    _error(f"Incorrect PIN ({retries_left} attempts remaining)")
                else:
                    _error(f"PIN error: {e}")
                raise
    ...
    def _load_index(self, pin_token=None) -> dict:
        """Load and decrypt the index"""
        if not os.path.exists(self.secrets_index):
            return {}
        with open(self.secrets_index, "rb") as f:
            data = f.read()
        if not data:
            return {}
        key = self.get_terces(("index:" + self.load_key_handle()).encode(), pin_token=pin_token)
        nonce, ciphertext = data[:12], data[12:]
        plaintext = AESGCM(key).decrypt(nonce, ciphertext, None)
        return json.loads(plaintext.decode())

    def _save_index(self, index: dict, pin_token=None):
        ...
        with open(self.secrets_index, "wb") as f:
